...
while True:
    ...
    detections = yolo_utils.filter_detections(detections, frame.shape[:2])
    ...
    for i in range(1, 3):  # vertical lines at 1/3 and 2/3 width
        x = int(i * frame.shape[1] / 3)
        cv2.line(frame, (x, 0), (x, frame.shape[0]), (0, 255, 0), 2)
    for j in range(1, 2):  # horizontal line at half height
        y = int(j * frame.shape[0] / 2)
